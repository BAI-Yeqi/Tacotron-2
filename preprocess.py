...
def preprocess(args, input_folders, out_dir, hparams):
	...
	metadata = preprocessor.build_from_path(hparams, input_folders, mel_dir,
											linear_dir, wav_dir, args.n_jobs,
											tqdm=tqdm)
...
def write_metadata(metadata, out_dir):
	...
	print('Write {} utterances, {} mel frames'.format(
		len(metadata), mel_frames))
	print('{} audio timesteps, ({:.2f} hours)'.format(
		timesteps, hours))
	print('Max input length (text chars): {}'.format(
		max(len(m[5]) for m in metadata)))
	print('Max mel frames length: {}'.format(
		max(int(m[4]) for m in metadata)))
	print('Max audio timesteps length: {}'.format(
		max(m[3] for m in metadata)))
